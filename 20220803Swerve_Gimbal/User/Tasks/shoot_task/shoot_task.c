...
//摩擦轮转速
#define FRICTION_SPEED_15 4320  
#define FRICTION_SPEED_18 4725  
#define FRICTION_SPEED_30 6700  
...
void Shoot_Feedback_Update(Shoot_Control_t *shoot_feedback)
{
	...
	for (i = 0;i < 2 ;i++)
	{
		...
		if(shoot_feedback->frictionSpeed[i].speed == shoot_feedback->frictionSpeed[i].rollingMotorMeasure->speed_rpm)
		{
		  shoot_feedback->frictionSpeed[i].speed_pid.Iout=0;
		}
	}
...
static void Shoot_Set_Control(Shoot_Control_t *shoot_set)
{
 ...
 if((rc_ctrl.rc.sleft != RC_DOWN&&rc_ctrl.rc.sleft != 0) || rc_ctrl.mouse.press_l == 1)
 {
		switch (shoot_set->shoot_speed_max)
		{
		case 15:
		shoot_set->frictionSpeed[0].speed_set = FRICTIONDIRECTION * FRICTION_SPEED_15 * 1; //4220
	  shoot_set->frictionSpeed[1].speed_set = FRICTIONDIRECTION * FRICTION_SPEED_15 * -1;  //4600   //4700
		break;
		case 18:
		shoot_set->frictionSpeed[0].speed_set = FRICTIONDIRECTION * FRICTION_SPEED_18 * 1;
	  shoot_set->frictionSpeed[1].speed_set = FRICTIONDIRECTION * FRICTION_SPEED_18 * -1;
		break;
		case 30:
		shoot_set->frictionSpeed[0].speed_set = FRICTIONDIRECTION * FRICTION_SPEED_30 * 1;
	  shoot_set->frictionSpeed[1].speed_set = FRICTIONDIRECTION * FRICTION_SPEED_30 * -1;  //7520   //7300
		break;
    ...
    if(rc_ctrl.rc.sleft == RC_DOWN ||(IF_KEY_PRESSED_V)  )
    {
